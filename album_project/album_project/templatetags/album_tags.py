...
Gallery = get_model('album_project', 'Gallery')
Photo = get_model('album_project', 'Photo')


@register.inclusion_tag('album_project/tags/next_in_gallery.html')
def next_in_gallery(photo, gallery):
    return {'photo': photo.get_next_in_gallery(gallery)}


@register.inclusion_tag('album_project/tags/prev_in_gallery.html')
...
def get_photo(parser, token):
    """Get a single photo from the album_project and return the img tag to display it.
...
def get_rotating_photo(parser, token):
    """Pick at random a photo from a given album_project gallery and return the img tag to display it.
